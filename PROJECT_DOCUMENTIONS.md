...
- Branch naming uses `aiqa/{requirement-slug}-{timestamp}-{random}` to avoid collisions when users create multiple PRs close together.
...
- The frontend safely handles partially loaded project/module/requirement data while creating a PR.
- The backend safely handles GitHub file creation responses even when GitHub does not return a `content.html_url` value.
- If GitHub does not return a direct file URL, the created pull request URL is still returned to the UI.

Recommended automation repository configuration:

- Repository owner
- Repository name
- Default branch
- Test folder path, for example `tests/e2e`
